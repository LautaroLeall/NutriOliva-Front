...
const STATUS_CONFIG = {
  ...
  // compatibilidad con valores legacy en espanol sin guion
  Activo: { dot: "bg-success", text: "Activo" },
...
export default function AdminPanel() {
  ...
  const { nutricionistas, loading, error, cambiarPlan, cambiarEstado } =
    useAdmin();
  ...
  const totalPatients = nutricionistas.reduce(
    (s, n) => s + (n.total_pacientes || 0),
    0,
  );
  const totalRevenue = activos.reduce(
    (s, n) => s + (n.precio_mensual_ars || 0),
    0,
  );
  ...
  async function guardarEdicion() {
    ...
    if (editForm.plan_suscripcion !== editingNutri.plan_suscripcion) {
      const { error: err } = await cambiarPlan(
        editingNutri.id,
        editForm.plan_suscripcion,
      );
      if (err) {
        toast.error("Error al cambiar el plan: " + err);
        huboError = true;
      }
    }

    if (!huboError && editForm.estado !== editingNutri.estado) {
      const { error: err } = await cambiarEstado(
        editingNutri.id,
        editForm.estado,
      );
      if (err) {
        toast.error("Error al cambiar el estado: " + err);
        huboError = true;
      }
    }
        ...
        <div className="card overflow-hidden">
          <div className="flex min-h-[600px]">
            ...
            <main className="flex-1 p-6 bg-white min-w-0">
              <div className="mb-5">
                <h2 className="font-display text-lg text-olive-dark">
                  Panel general
                </h2>
              ...
              <div className="grid grid-cols-3 gap-3 mb-5">
                {metrics.map((m) => (
                  <div
                    key={m.label}
                    className="bg-cream border border-cream-darker rounded-xl p-4"
                  >
                    ...
                    <div className="text-[10px] text-success mt-1 font-display">
                      {m.sub}
                    </div>
                  </div>
                ))}
              ...
              <div className="border border-cream-darker rounded-xl overflow-hidden">
                <div className="flex justify-between items-center px-4 py-3 bg-cream border-b border-cream-darker">
                  ...
                  <span className="text-[10px] text-muted font-display italic">
                    Alta de nutricionistas disponible proxima version
                  </span>
                </div>
                ...
                {loading ? (
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr>
                        {["Profesional", "Plan", "Pacientes", "Estado", ""].map(
                          (h) => (
                            <th
                              key={h}
                    ...
                    <tbody>
                      {nutricionistas.map((n) => {
                        const st =
                          STATUS_CONFIG[n.estado] || STATUS_CONFIG["Activo"];
                        const planKey = (
                          n.plan_suscripcion || "starter"
                        ).toLowerCase();
                          ...
                          >
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2.5">
                                <span className="avatar text-[10px]">
                                  {initials(n.nombre)}
                                </span>
                                <div>
                                  <div className="font-semibold text-olive-dark text-[12px]">
                                    {n.nombre}
                                  </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) =>
                    setEditForm((f) => ({
                      ...f,
                      plan_suscripcion: e.target.value,
                    }))
                  }
                >
                  <option value="starter">
                    Starter — hasta 15 pac. · {formatARS(75000)}/mes
                  </option>
                  <option value="pro">
                    Pro — hasta 50 pac. · {formatARS(125000)}/mes
                  </option>
                  <option value="clinic">
                    Clinic — ilimitado · {formatARS(150000)}/mes
                  </option>
                </select>
              </div>
              <div>
