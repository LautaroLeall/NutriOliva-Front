...
function BalanceCard({ balance, caloriasObjetivo, loading }) {
  const { consumidas, gastadas, netas, restante, pct, barColor, estaEnExceso } = balance
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="flex justify-between mt-1.5">
            ...
            <span className={`text-[9.5px] font-display font-medium
...
export default function PatientPanel() {
  ...
  const {
    comidas, actividades, loading: loadingReg,
    agregarComida, editarComida, eliminarComida,
    agregarActividad, eliminarActividad,
  } = useRegistros(pacienteId, fecha)

  const balance = useBalance(comidas, actividades, planActivo?.calorias_objetivo || 2000)
  ...
  const comidasPlan = [...(planActivo?.comidas_plan || [])].sort(
    (a, b) => (ORDEN_TIPO[a.tipo_comida] || 9) - (ORDEN_TIPO[b.tipo_comida] || 9)
  )

  return (
    <div className="min-h-screen bg-[#EFEAE0] pb-32">
            ...
            {loadingInf ? (
              ...
              <>
                ...
                {(['desayuno', 'almuerzo', 'merienda', 'cena', 'snack']).map(tipo => {
                  ...
                  return (
                    <div key={tipo} className="bg-white rounded-2xl border border-cream-darker overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-3 border-b border-cream">
                        ...
                        {totalCal > 0 && (
                          <span className="text-[10.5px] text-muted">{totalCal} kcal</span>
                        )}
                      </div>
                      <div className="divide-y divide-cream">
                        {items.map(c => (
                          <div key={c.id} className="px-4 py-2.5">
                            <p className="text-[12px] text-olive-dark">{c.descripcion}</p>
                            {c.calorias_aprox && (
