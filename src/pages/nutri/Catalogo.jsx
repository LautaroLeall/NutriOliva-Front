...
import {
  ArrowLeft, Plus, Search, Pencil, Trash2, Loader2, AlertCircle,
} from 'lucide-react'
    ...
    <div className="page min-h-screen bg-[#EFEAE0]">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex justify-between items-start mb-6">
          <div>
            <h2 className="font-display text-xl text-olive-dark">Catalogo de alimentos</h2>
        ...
        <div className="card overflow-hidden">
          ...
          {!loading && !error && filtrados.length > 0 && (
            <table className="w-full border-collapse">
              ...
              <tbody>
                {filtrados.map(a => (
                  <tr key={a.id} className="border-b border-cream last:border-0 hover:bg-cream/40 transition-colors">
                    <td className="px-5 py-3">
                      <span className="font-medium text-olive-dark text-sm">{a.nombre}</span>
                    </td>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              {errors.calorias_por_unidad && (
                <p className="text-[10.5px] text-red-500 mt-1">{errors.calorias_por_unidad}</p>
              )}
            </div>
            <div>
              <label className="label">Unidad</label>
              <select name="unidad" className="input" value={form.unidad} onChange={handleChange}>
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              <div key={name}>
                ...
                />
                {errors[name] && (
                  <p className="text-[10.5px] text-red-500 mt-1">{errors[name]}</p>
                )}
              </div>
            ))}
          </div>
